...
import {
	...
	ChordLink,
	ChordNode,
	EMPTY_SIMPLE_SCATTER_CONFIG,
	HeatmapData,
	SimpleScatterConfig,
} from 'src/app/types/charts';
...
export class GradeRelationshipComponent implements OnChanges {
	...
	countRelations(uid1: string, uid2: string): number {
		...
		const songs2 = Array.from(this.songs.values())
			.filter((song) => song.user === uid2)
			.map((song) => song.id);

		return (
			songs2.filter((song) => favorites1.includes(song)).length +
			songs2.filter(
				(song) => results1.mean < (results1.data.values.get(song) || -1)
			).length
		);
	}
		...
		for (let i = 0; i < users.length; i++) {
			...
			for (let j = 0; j < users.length; j++) {
				...
				this.heatmapData.push([
					i,
					j,
					user1 === user2 ? 0 : this.countRelations(user1, user2),
				]);
			}
		}
	}
	...
	generateSimpleScatterConfig(): void {
		...
		this.scatterConfig = {
			data: keepUniqueValues(data).map((value) => [
				value[0],
				value[1],
				counts.get(value.join(',')) ?? 0,
			]),
			color: '#693BB8',
			formatter: (value) => {
	...
	getSimilarity(): string {
		...
		const nSongs =
			this.xUser === this.yUser
				? this.songs.size - nSongsPerUser
				: this.songs.size - 2 * nSongsPerUser;

		// check if values[0] and values[1] have the same sign
		return (
			(
				(this.scatterConfig.data.filter((values) => {
