...
import URLParse from 'url-parse'; // Importation corrigée
...
export function getIDFromURL(song: Song): string {
	switch (song.platform) {
		case SongPlatform.YOUTUBE: {
			const parsedURL = new URLParse(song.url, true); // Utilisation correcte
		...
		case SongPlatform.PEERTUBE:
			return song.url.split('/videos/watch/').pop() ?? '';
...
export function URLToSongPlatform(url: string): SongPlatform {
	const hostname = new URLParse(url, true).hostname; // Utilisation correcte
